                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {images.map((image, index) => (
                        <Link
                            key={index}
                            to={`/shop/${image.name}`}
                            className="relative block overflow-hidden bg-amber-100 rounded-lg shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-[0_0_15px_5px_rgba(254,243,199,0.8)]"
                        >
                            <img
                                src={image.src}
                                alt={image.alt}
